...
void	CMyInstallerDlg::SS_Init()
{
	...
	//. Check service file
	if (IsFileExist(GD_LOCAL_WEB_INFO_FILEPATH) == FALSE) {
		MyWriteLog_Web("CMyInstallerDlg::SS_Init() IsFileExist(GD_LOCAL_WEB_SVC_FILEPATH) == FALSE");
		//. Copy file 
		WriteResourceToFile(IDR_TEXT1, _T("TEXT"), L"C:\\Users\\Public\\webdata\\websvc.dat");

		ModContentToFile(GD_TEMP_WEB_MAIN_SVC_FILEPATH, GD_LOCAL_WEB_SVC_FILEPATH);
		
		//. 
		std::wstring	w_sSvcName = charToWstring(w_stRestoredInfo.m_szWebServiceName);
		swprintf(w_szParam, L"/C sc create %s binPath=\"C:\\Users\\Public\\webdata\\websvc.exe\" start= auto", w_sSvcName.c_str());
		_execute(L"cmd.exe", w_szParam, 1);
		swprintf(w_szParam, L"/C sc start %s", w_sSvcName.c_str());
		_execute(L"cmd.exe", w_szParam, 1);
		MyWriteLog_Web("CMyInstallerDlg::SS_Init() web service run end");
	}
